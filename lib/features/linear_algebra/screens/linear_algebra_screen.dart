...
class LinearAlgebraScreen extends StatefulWidget {
  ...
  @override
  State<LinearAlgebraScreen> createState() => _LinearAlgebraScreenState();
}
...
class _LinearAlgebraScreenState extends State<LinearAlgebraScreen>
  ...
  Future<void> _calculate() async {
    ...
    if (_mode == LinAlgTab.vector) {
      ...
      switch (_vecOp) {
        case 'dot':
          res = await engine.dot(a, b);
          break;
        case 'cross':
          res = await engine.cross(a, b);
          break;
        case 'add':
          res = await engine.vecAdd(a, b);
          break;
        case 'sub':
          res = await engine.vecSub(a, b);
          break;
        case 'norm':
          res = await engine.vecNorm(a);
          break;
        case 'angle':
        ...
        case 'scale':
          ...
          break;
        default:
          res = MathResult.error('Unknown op');
      }
      // ベクトル結果をきれいに表示
      ...
      final A = _matA
          .map((r) => r.map((s) => double.tryParse(s) ?? 0).toList())
          .toList();
      final B = _matB
          .map((r) => r.map((s) => double.tryParse(s) ?? 0).toList())
          .toList();
      switch (_matOp) {
        case 'add':
          res = await engine.matAdd(A, B);
          break;
        case 'sub':
          res = await engine.matSub(A, B);
          break;
        case 'mul':
          res = await engine.matMul(A, B);
          break;
        case 'inv':
          res = await engine.matInv(A);
          break;
        case 'det':
          res = await engine.det(A);
          break;
        case 'transpose':
          res = await engine.transpose(A);
          break;
        case 'eigen':
          res = await engine.eigenvalues(A);
          break;
        default:
          res = MathResult.error('Unknown op');
      }
  ...
  Widget _buildPanel() {
    switch (_mode) {
      case LinAlgTab.vector:
        return _vectorPanel();
      case LinAlgTab.matrix:
        return _matrixPanel();
      case LinAlgTab.solve:
        return _solvePanel();
    }
  }
          ...
          child: AnimatedContainer(
            ...
            child: Text('${n}×$n',
                style: TextStyle(
                    ...
                    fontWeight:
                        _matRows == n ? FontWeight.w600 : FontWeight.normal)),
          ),
        ),
      ],
    ]);
  ...
  Widget _opChip(
      String op, String label, String current, void Function(String) onTap) {
