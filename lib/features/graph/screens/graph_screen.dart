...
class _GraphScreenState extends State<GraphScreen> {
  ...
  Future<void> _plot() async {
    ...
    final exprs =
        _plots.map((p) => MathEngineService.preprocess(p.expr)).toList();
              ...
              child: Row(children: [
                ...
                Expanded(
                  ...
                  controller: TextEditingController(text: e.value.expr)
                    ..selection =
                        TextSelection.collapsed(offset: e.value.expr.length),
                  decoration: InputDecoration(
                    ...
                    border: OutlineInputBorder(
                        ...
                        borderSide:
                            BorderSide(color: e.value.color.withOpacity(0.3))),
                    ...
                    focusedBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(10),
                        borderSide:
                            BorderSide(color: e.value.color, width: 1.5)),
          ...
          child: Container(
            ...
            child: _loading
                ...
                : _error.isNotEmpty
                    ...
                    : _plotData.isEmpty
                        ? Center(
                            child: Text('関数を入力して「描画」を押してください',
                                style: const TextStyle(
                                    fontFamily: 'Space Grotesk',
...
class _GraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = (xMin / xStep).ceil() * xStep; x <= xMax; x += xStep) {
      ...
      _drawText(canvas, x == 0 ? '' : _fmt(x), Offset(sx + 2, size.height - 14),
          axisLabelStyle);
    }
    ...
    for (double y = (yMin / yStep).ceil() * yStep; y <= yMax; y += yStep) {
      ...
      _drawText(
          canvas, y == 0 ? '' : _fmt(y), Offset(4, sy - 12), axisLabelStyle);
    }
  ...
  double _niceStep(double rough) {
    final magnitude =
        (rough == 0) ? 1.0 : pow10(rough.abs().log10().floorVal());
