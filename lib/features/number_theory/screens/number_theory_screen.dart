...
class NumberTheoryScreen extends StatefulWidget {
  ...
  @override
  State<NumberTheoryScreen> createState() => _NumberTheoryScreenState();
}
...
class _NumberTheoryScreenState extends State<NumberTheoryScreen>
  ...
  Future<void> _calculate() async {
    ...
    switch (_mode) {
      ...
      case NumThTab.baseConvert:
        ...
        if (res.success) {
          res = MathResult(
              success: true,
              result:
                  '${_f['value']!} ($_fromBase進) = ${res.result} ($_toBase進)');
        }
        break;
      case NumThTab.primeCheck:
        ...
        if (sieveRes.success) {
          try {
            _primeList = jsonDecode(sieveRes.result) as List;
          } catch (_) {}
        }
        res = MathResult(
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(
            spacing: 6,
            runSpacing: 6,
            children: _primeList.take(46).map((p) {
              final n = int.tryParse(_f['n']!) ?? 0;
              final isTarget = p == n;
              return Container(
                ...
                child: Text('$p',
                    style: TextStyle(
                        ...
                        fontWeight:
                            isTarget ? FontWeight.w700 : FontWeight.normal)),
              );
            }).toList()),
      ]),
    );
  ...
  Widget _buildKeyboard() {
    ...
    final key2 =
        (_mode == NumThTab.gcdlcm || _mode == NumThTab.euclid) ? 'b' : null;
    return Padding(
      ...
      child: Column(children: [
        ...
        _kr([
          _k('0', () => _ap(key1, '0'), flex: 2),
          _k('00', () => _ap(key1, '00')),
          _k('計算', _calculate, a: true)
        ]),
      ]),
    );
  }
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(12),
                  border:
                      Border.all(color: AppTheme.borderColor.withOpacity(0.4))),
              child: Center(
                  child: _loading && a
                      ? const SizedBox(
                          width: 18,
