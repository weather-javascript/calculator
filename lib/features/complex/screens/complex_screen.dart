...
class ComplexScreen extends StatefulWidget {
  ...
  @override State<ComplexScreen> createState() => _ComplexScreenState();
}
...
class _ComplexScreenState extends State<ComplexScreen>
  ...
  void _ap(String v) => setState(() => _f[_activeField] = (_f[_activeField]! + v));
  void _bs() => setState(() { final c=_f[_activeField]!; if(c.isNotEmpty) _f[_activeField]=c.substring(0,c.length-1); });
  ...
  Future<void> _calculate() async {
    ...
    if (_mode == ComplexTab.eval) {
      ...
      if (_operation == 'fromPolar') {
        res = await engine.complexFromPolar(_d('r'), _d('theta'), unit: _polarUnit);
        if (res.success) {
          try {
            final j = jsonDecode(res.result) as Map<String, dynamic>;
            res = MathResult(success: true,
              result: '直交形式: ${j['re']} + ${j['im']}i');
          } catch (_) {}
        }
      } else {
      ...
      switch (_operation) {
        case 'add':   res = await engine.complexAdd(_d('re1'),_d('im1'),_d('re2'),_d('im2')); break;
        case 'sub':   res = await engine.complexSub(_d('re1'),_d('im1'),_d('re2'),_d('im2')); break;
        case 'mul':   res = await engine.complexMul(_d('re1'),_d('im1'),_d('re2'),_d('im2')); break;
        case 'div':   res = await engine.complexDiv(_d('re1'),_d('im1'),_d('re2'),_d('im2')); break;
        case 'abs':   res = await engine.complexAbs(_d('re1'),_d('im1')); break;
        case 'arg':   res = await engine.complexArg(_d('re1'),_d('im1')); break;
        case 'conj':  res = await engine.complexConj(_d('re1'),_d('im1')); break;
        case 'pow':   res = await engine.complexPow(_d('re1'),_d('im1'),_d('re2')); break;
        case 'sqrt':  res = await engine.complexSqrt(_d('re1'),_d('im1')); break;
        default: res = MathResult.error('Unknown operation');
      }
    }
  ...
  Widget _buildPanel() {
    switch (_mode) {
      case ComplexTab.arithmetic: return _arithmeticPanel();
      case ComplexTab.polar: return _polarPanel();
      case ComplexTab.eval: return _evalPanel();
    }
  }
  ...
  Widget _arithmeticPanel() {
    ...
    return Column(children: [
      ...
      Wrap(spacing: 6, children: [
        for (final op in ['add','sub','mul','div','abs','arg','conj','pow','sqrt'])
      ...
      Row(children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
          child: Text('i', style: TextStyle(color: _color, fontSize: 16, fontWeight: FontWeight.bold))),
      ]),
      if (needs2) ...[
        const SizedBox(height: 6),
        Row(children: [
          Expanded(child: _field(
            _operation == 'pow' ? '指数 n' : 'z₂ 実部',
  ...
  Widget _polarPanel() {
    return Column(children: [
      Row(children: [
        _toggleOp('直交→極', 'toPolar'), const SizedBox(width: 8),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: TextStyle(fontSize: 10, color: active ? _color : AppTheme.textMuted,
            fontFamily: 'Space Grotesk', fontWeight: FontWeight.w600)),
          const SizedBox(height: 2),
          Text(_f[key]!.isEmpty ? '0' : _f[key]!, style: const TextStyle(
            fontFamily: 'IBM Plex Mono', fontSize: 15, color: AppTheme.textPrimary),
  ...
  Widget _opChip(String op, String label) {
    ...
    return GestureDetector(
      onTap: () => setState(() { _operation = op; _result = ''; }),
      child: AnimatedContainer(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: active ? _color.withOpacity(0.15) : AppTheme.surfaceCard,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: active ? _color.withOpacity(0.5) : AppTheme.borderColor,
            width: active ? 1.5 : 1)),
        child: Text(label, style: TextStyle(
          fontFamily: 'Space Grotesk', fontSize: 12, color: active ? _color : AppTheme.textMuted)),
      ),
    );
  }

  Widget _unitToggle() {
    return Row(mainAxisSize: MainAxisSize.min, children: ['rad','deg'].map((u) {
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: active ? _color.withOpacity(0.4) : AppTheme.borderColor)),
          child: Text(u, style: TextStyle(fontSize: 11, color: active ? _color : AppTheme.textMuted,
  ...
  Widget _buildKeyboard() {
    return Padding(
      ...
      child: Column(children: [
        _kr([_k('AC',(){setState((){_f.updateAll((_,__)=>'');_result='';});},r:true),
          _k('⌫',_bs,o:true), _k('.',()=>_ap('.')), _k('-',()=>_ap('-'),o:true)]),
        _kr([_k('7',()=>_ap('7')),_k('8',()=>_ap('8')),_k('9',()=>_ap('9')),_k('÷',()=>_ap('/'),o:true)]),
        _kr([_k('4',()=>_ap('4')),_k('5',()=>_ap('5')),_k('6',()=>_ap('6')),_k('×',()=>_ap('*'),o:true)]),
        _kr([_k('1',()=>_ap('1')),_k('2',()=>_ap('2')),_k('3',()=>_ap('3')),_k('+',()=>_ap('+'),o:true)]),
        _kr([_k('0',()=>_ap('0'),f:2),_k('計算',_calculate,a:true,f:2)]),
      ]),
    );
  }
  ...
  Widget _k(String lbl, VoidCallback fn, {bool r=false,bool o=false,bool a=false,int f=1}) {
    Color bg,fg;
    if(a){bg=_color.withOpacity(0.85);fg=AppTheme.textPrimary;}
