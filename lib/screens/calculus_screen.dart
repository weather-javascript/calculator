...
class CalculusScreen extends StatefulWidget {
  ...
  @override State<CalculusScreen> createState() => _CalculusScreenState();
}
...
class _CalculusScreenState extends State<CalculusScreen>
  ...
  Future<void> _calcDiff(BuildContext ctx) async {
    ...
    final res = await engine.differentiate(MathEngineService.preprocess(expr), v: v);
  ...
  Future<void> _calcInt(BuildContext ctx) async {
    ...
    if (lo.isEmpty || hi.isEmpty) {
      s.setResult('∫($expr)d$v',
        '不定積分: ∫($expr)d$v + C\n（定積分は下限・上限を入力してください）');
    } else {
      final a = double.tryParse(lo), b = double.tryParse(hi);
      if (a == null || b == null) { s.setResult('', 'Error: 積分範囲が無効', isError: true); return; }
    ...
    return Consumer<CalculatorState>(builder: (ctx, s, _) => Column(children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(
          color: AppTheme.surfaceCard, borderRadius: BorderRadius.circular(12)),
        child: TabBar(
          controller: _tab,
          indicator: BoxDecoration(
            color: _color.withOpacity(0.2), borderRadius: BorderRadius.circular(10),
  ...
  Widget _diffInputs(CalculatorState s) => Column(children: [
    InputField(label: 'f(x) — 微分する関数', value: s.field1,
      isActive: s.activeField == 'field1',
      onTap: () => s.setActiveField('field1'),
      placeholder: '例: x^3 + 2*x', accentColor: _color),
    const SizedBox(height: 8),
    InputField(label: '変数', value: s.field2,
      isActive: s.activeField == 'field2',
      onTap: () => s.setActiveField('field2'),
      placeholder: 'x', accentColor: _color),
  ]);

  Widget _intInputs(CalculatorState s) => Column(children: [
    InputField(label: 'f(x) — 積分する関数', value: s.field1,
      isActive: s.activeField == 'field1',
      onTap: () => s.setActiveField('field1'),
      placeholder: '例: x^2, sin(x)', accentColor: _color),
    const SizedBox(height: 8),
    Row(children: [
      ...
      Expanded(child: InputField(label: '下限 a', value: s.field3,
        isActive: s.activeField == 'field3',
        onTap: () => s.setActiveField('field3'),
        placeholder: '省略=不定積分', accentColor: _color)),
      const SizedBox(width: 8),
      Expanded(child: InputField(label: '上限 b', value: s.field4,
      ...
      child: Column(children: [
        ...
        _r([_b('0', cb.ButtonStyle.number, () => ap('0')),
            ...
            _b(_isDiff ? 'd/dx' : '∫', cb.ButtonStyle.action,
              () => _isDiff ? _calcDiff(ctx) : _calcInt(ctx), fs: 14)]),
      ]),
    );
  }
