...
class BasicScreen extends StatelessWidget {
  ...
  Future<void> _calc(BuildContext context) async {
    ...
    final res = await MathEngineService().evaluate(MathEngineService.preprocess(raw));
    state.setResult(raw,
  ...
  Widget build(BuildContext context) {
    return Consumer<CalculatorState>(builder: (ctx, state, _) {
      switch (state.mode) {
        case CalcMode.applied:   return _applied(ctx, state);
        case CalcMode.functions: return _functions(ctx, state);
        default:                 return _basic(ctx, state);
      }
    });
  }
  ...
  Widget _functions(BuildContext ctx, CalculatorState s) => _pad([
    ...
    _row([
      _b('log', cb.ButtonStyle.special,
          () => s.inputFunction('log10('), sublabel: 'log₁₀'),
      _b('ln',  cb.ButtonStyle.special,
          () => s.inputFunction('log('),   sublabel: 'logₑ'),
      _b('exp', cb.ButtonStyle.special,
          () => s.inputFunction('exp('),   sublabel: 'eˣ'),
  ...
  Widget _row(List<Widget> btns) => Padding(
    ...
    child: Row(children: btns.map((b) => Expanded(child: Padding(
      padding: const EdgeInsets.symmetric(horizontal: 4), child: b))).toList()));

  Widget _b(String label, cb.ButtonStyle style, VoidCallback onTap,
      {String? sublabel}) =>
    cb.CalcButton(label: label, sublabel: sublabel,
      style: style, onTap: onTap, height: 62);
