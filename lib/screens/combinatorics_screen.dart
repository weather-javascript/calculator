...
class CombinatoricsScreen extends StatefulWidget {
  ...
  @override State<CombinatoricsScreen> createState() => _CombinatoricsScreenState();
}
...
class _CombinatoricsScreenState extends State<CombinatoricsScreen> {
  ...
  Future<void> _calc(BuildContext ctx) async {
    ...
    if (_cm == _CombiMode.factorial) {
      ...
      if (n == null) { s.setResult('n!', 'Error: 整数を入力', isError: true); return; }
      final r = await engine.factorial(n);
      s.setResult('${s.field1}!', r.success ? r.result : r.result, isError: !r.success);
    } else {
      final n = int.tryParse(s.field1.trim()), r2 = int.tryParse(s.field2.trim());
      if (n == null || r2 == null) { s.setResult('', 'Error: 整数を入力', isError: true); return; }
      ...
      final label = _cm == _CombiMode.permutation
          ? '${s.field1}P${s.field2}' : '${s.field1}C${s.field2}';
      s.setResult(label, res.success ? res.result : res.result, isError: !res.success);
    }
  }
    ...
    return Consumer<CalculatorState>(builder: (ctx, s, _) => Column(children: [
      ...
      Padding(
        ...
        child: Container(
          ...
          child: Text(
            _cm == _CombiMode.permutation ? 'nPr = n! / (n-r)!'
              : _cm == _CombiMode.combination ? 'nCr = n! / (r! × (n-r)!)'
              : 'n! = n × (n-1) × ⋯ × 1',
      ...
      Padding(
        ...
        child: _cm == _CombiMode.factorial
          ? InputField(label: '整数 n', value: s.field1,
              isActive: s.activeField == 'field1',
              onTap: () => s.setActiveField('field1'),
              placeholder: '例: 10', accentColor: _color)
          : Row(children: [
              Expanded(child: InputField(label: 'n — 総数', value: s.field1,
                isActive: s.activeField == 'field1',
                onTap: () => s.setActiveField('field1'),
                placeholder: '例: 10', accentColor: _color)),
              const SizedBox(width: 8),
              Expanded(child: InputField(label: 'r — 選ぶ数', value: s.field2,
                isActive: s.activeField == 'field2',
                onTap: () => s.setActiveField('field2'),
                placeholder: '例: 3', accentColor: _color)),
            ])),
      // キーボード
  ...
  Widget _chip(String label, _CombiMode m) {
    ...
    return Expanded(child: GestureDetector(
      onTap: () { setState(() => _cm = m); context.read<CalculatorState>().clearAll(); },
      child: AnimatedContainer(
        ...
        child: Text(label, textAlign: TextAlign.center,
          style: TextStyle(fontFamily: 'IBM Plex Mono', fontSize: 13,
            ...
            color: active ? _color : AppTheme.textMuted, height: 1.4)),
      ),
    ));
  }
      ...
      child: Column(children: [
        _r([_b('AC', cb.ButtonStyle.clear, s.clearAll),
            ...
            _cm != _CombiMode.factorial
              ? _b('→r', cb.ButtonStyle.function, () => s.setActiveField('field2'))
              : _b('', cb.ButtonStyle.number, () {})]),
        _r([_b('7', cb.ButtonStyle.number, () => s.appendToActive('7')),
