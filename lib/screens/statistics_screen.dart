...
class _StatisticsScreenState extends State<StatisticsScreen> {
  ...
  Future<void> _calc() async {
    ...
    try {
      ...
      setState(() {
        _loading = false;
        _stats = j.map((k, v) => MapEntry(k, v.toString()));
      });
    } catch (e) {
      setState(() {
        _loading = false;
  ...
  Widget _results() {
    ...
    final items = [
      ...
      ('合計 Σ', _stats!['sum']!),
      ('平均 x̄', _stats!['mean']!),
      ('中央値 Me', _stats!['median']!),
      ('最頻値 Mo', _stats!['mode']!),
      ('分散 σ²', _stats!['variance']!),
      ('標準偏差 σ', _stats!['stdDev']!),
      ('最小値', _stats!['min']!),
      ('最大値', _stats!['max']!),
      ('範囲', _stats!['range']!),
      ('第1四分位 Q₁', _stats!['q1']!),
  ...
  Widget _r(List<String> digits) => Row(
      children: digits
          .map((d) => Expanded(
              child: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 3),
